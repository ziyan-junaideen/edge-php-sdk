...
class PaymentDemandTest extends TestCase
{
    ...
    private function client(array $responses)
    {
        ...
        return new ApiClient('ept_sandbox_s_test', [
            ...
            'user_agent_suffix' => 'DemandTest/1.0',
        ]);
    }
    ...
    {
        $id = 'a/b ?#%';
        $raw = ['type' => 'payment_demands', 'id' => $id];
        ...
        $query = [
            ...
            'fields' => ['payment_demands' => ['amount_cents', 'amount_currency', 'buyer'], 'customers' => 'name'],
            'filter' => ['buyer' => ['email' => 'ada@example.com']], 'page' => ['size' => 25, 'number' => 2],
        ];
        $args = [$client];
        if (in_array($operation, ['show', 'update', 'confirm'], true)) {
            $args[] = $useResource ? new PaymentDemand($raw) : $id;
        }
        $expected = ['type' => 'payment_demands'];
        ...
        if (in_array($operation, ['create', 'update'], true)) {
            $attributes = $operation === 'create' ? [
                ...
                'idempotency_key' => 'order-test-1', 'capture_method' => 'automatic',
        ...
        $this->assertStringContainsString('DemandTest/1.0', $request->getHeaderLine('User-Agent'));
        ...
        $this->assertInstanceOf(PaymentDemand::class, $operation === 'list' ? $result->data[0] : $result->data);
    ...
    public function testRichValuesEmbeddedDataAndRelationshipsArePreserved()
    {
        $attributes = [
            ...
            'succeeded_at' => '2026-09-13T10:01:00Z', 'processor_state' => 'future_state',
            'capture_method' => 'future_capture', 'purchase_kind' => 'future_kind', 'confirmed' => true,
        ...
        $relationships['payment_subscription'] = ['data' => ['type' => 'payment_subscriptions', 'id' => 'subscription-1']];
        $raw = ['type' => 'payment_demands', 'id' => '1', 'attributes' => $attributes,
            'relationships' => $relationships, 'meta' => ['example' => true], 'links' => (object) [], 'extension' => true];
        $document = ['data' => $raw, 'included' => $included, 'meta' => ['example' => true], 'links' => ['self' => '/v2/payment_demands/1']];
        $client = $this->client([new Response(200, ['X-Request-Id' => 'demand-request'], json_encode($document))]);
        $result = PaymentDemand::show($client, '1');
        $demand = $result->data;
        $this->assertEquals(new Money(2500, 'USD'), $demand->amount);
        $this->assertEquals(new Money(75, 'USD'), $demand->fee);
        foreach (['created_at', 'updated_at', 'succeeded_at'] as $field) {
            $this->assertInstanceOf(\DateTimeImmutable::class, $demand->$field);
        }
        $this->assertSame('123456', $demand->created_at->format('u'));
        foreach (array_diff(array_keys($attributes), ['created_at', 'updated_at', 'succeeded_at']) as $field) {
            $this->assertEquals(json_decode(json_encode($attributes[$field])), $demand->$field);
        }
        foreach (['discount', 'total_collected', 'amount_refunded', 'fee_currency'] as $field) {
            $this->assertEquals(new Unavailable(Unavailable::UNDEFINED), $demand->$field);
        }
        foreach (['buyer', 'payer', 'receiver'] as $name) {
            $this->assertInstanceOf(Customer::class, $demand->getRelated($name));
        }
        $this->assertInstanceOf(PaymentMethod::class, $demand->getRelated('payment_method'));
        $this->assertInstanceOf(ConsumerAddress::class, $demand->getRelated('billing_address'));
        $this->assertNull($demand->getRelated('shipping_address'));
        $this->assertInstanceOf(Linkage::class, $demand->getRelated('merchant'));
        $this->assertInstanceOf(Linkage::class, $demand->getRelated('payment_subscription'));
        $this->assertEquals(json_decode(json_encode($raw)), $demand->getRaw());
        $this->assertEquals(json_decode(json_encode($document)), $result->raw);
        $this->assertTrue($result->meta->example);
        $this->assertSame('/v2/payment_demands/1', $result->links->self);
        $this->assertSame(['demand-request'], $result->headers['X-Request-Id']);
        $this->assertCount(1, $this->history);
        $this->expectException(\LogicException::class);
        $demand->processor_state = 'succeeded';
    }

    public function testIncludedDemandsUseMoneyDatesSparseFieldsAndLocalOverrides()
    {
        $response = new \Edge\Response(new Response(200, [], json_encode([
            'data' => ['type' => 'customers', 'id' => '1', 'relationships' => [
                'payment_demands' => ['data' => [['type' => 'payment_demands', 'id' => '2']]],
            ]],
        ...
        $result = (new ResourceDecoder())->decode($response, ['fields' => ['payment_demands' => $selected]]);
        $demand = $result->data->getRelated('payment_demands')[0];
        $this->assertInstanceOf(PaymentDemand::class, $demand);
        $this->assertSame($result->included[0], $demand);
        $this->assertEquals(new Money(2500, 'USD'), $demand->amount);
        $this->assertEquals(new Money(75, 'USD'), $demand->fee);
        $this->assertInstanceOf(\DateTimeImmutable::class, $demand->created_at);
        foreach (array_diff(PaymentDemand::FIELDS, $selected) as $field) {
            $this->assertEquals(new Unavailable(Unavailable::UNFETCHED), $demand->$field);
        }
        $custom = (new ResourceDecoder())->register('payment_demands', DecodedResource::class)->decode($response);
        $this->assertInstanceOf(DecodedResource::class, $custom->included[0]);
        $this->assertInstanceOf(PaymentDemand::class, (new ResourceResult($response))->included[0]);
    }
    ...
    public function testMoneyDateFailuresNullsAndSparseOmissionsRemainDistinct()
    {
        $records = [
            ...
            ['amount_cents' => null, 'amount_currency' => null, 'fee_cents' => null, 'succeeded_at' => null],
            ['amount_cents' => '100', 'amount_currency' => 'USD', 'fee_cents' => 5],
            [],
        ];
        $client = $this->client(array_map(function ($attributes) {
            return new Response(200, [], json_encode(['data' => ['type' => 'payment_demands', 'id' => '1', 'attributes' => (object) $attributes]]));
        }, $records));
        $first = PaymentDemand::show($client, '1')->data;
        ...
        $second = PaymentDemand::show($client, '1')->data;
        $this->assertNull($second->amount);
        $this->assertNull($second->fee);
        $this->assertNull($second->succeeded_at);
        $third = PaymentDemand::show($client, '1')->data;
        $this->assertSame(Unavailable::DECODING_FAILURE, $third->amount->reason);
        $this->assertEquals(new Money(5, 'USD'), $third->fee);
        $fourth = PaymentDemand::show($client, '1', ['fields' => ['payment_demands' => 'description']])->data;
    ...
    {
        $client = $this->client([new Response(200, [], '{"data":[],"links":{"next":"https://api.tryedge.io/v2/payment_demands?page[number]=2"}}'), new Response(204)]);
        $page = PaymentDemand::list($client);
        $this->assertSame([], $page->data);
        $this->assertSame('https://api.tryedge.io/v2/payment_demands?page[number]=2', $page->links->next);
        $this->assertCount(1, $this->history);
        $this->assertNull(PaymentDemand::confirm($client, '1')->data);
        foreach (['show', 'update', 'confirm'] as $operation) {
            foreach ([new Customer(['type' => 'payment_demands', 'id' => '1']), new PaymentDemand(['type' => 'customers', 'id' => '1'])] as $resource) {
                try {
                    PaymentDemand::$operation($client, $resource);
        ...
        $this->assertFalse(method_exists(PaymentDemand::class, 'delete'));
    }
